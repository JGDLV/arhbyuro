// let headerHeight = $('.header__top').innerHeight();
...
$(document).ready(function () {
	...
	$('.icon-toggle').click(function () {
		...
		let headerMobileHeight = $('.header__top').innerHeight();
		if ($('.menu').hasClass('active')) {
			$('.menu').css('top', headerMobileHeight + 'px');
		} else {
			$('.menu').css('top', -100 + '%');
		}
		// $('.menu').css('top', -100 + '%');
		// $('.menu.active').css('top', headerMobileHeight + 'px');
		// return false;
	});

	$('.menu__link').on('click', function() {
		$('.menu, .icon-toggle').removeClass('active');
		$('.menu').css('top', -100 + '%');
	});
	...
	// $(document).on('click', '.header .goto', function (event) {
	...
	let headerHeight = $('.intro').offset().top
	console.log(headerHeight)

	$(window).scroll(function () {
		$(this).scrollTop() > headerHeight
			? ($('.header_fixed').addClass('stickytop'))
			: ($('.header_fixed').removeClass('stickytop'));
	});
	...
	$(window).on('load resize', function () {
		if ($(window).width() <= 1199) {
			let headerMobileHeight = $('.header__top').innerHeight();
			$('body').css('padding-top', headerMobileHeight);
		} else {
